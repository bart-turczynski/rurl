...
(`get_port()`, `get_query()`, `get_fragment()`, `get_userinfo()`)
instead. The query can be selectively kept on `clean_url` via
`query_handling` on either `safe_parse_url()` or `get_clean_url()` (see
[Query handling](#query-handling)). With `path_encoding = "decode"` the
path is shown decoded, so `clean_url` is human-readable rather than
guaranteed URL-safe.
...
`get_query()` takes the same arguments (defaulting to
`query_handling = "keep"`) to pull a cleaned query directly:
...
Audit a URL set before choosing a policy with `query_param_summary()`,
...
and `puny_decode`. The `full_parse` cache is bounded at 100000 entries
by default: a hard reset-watermark that caps peak memory (once the bound
is hit the cache empties completely and rebuilds, rather than evicting
per entry). Set `max_full_parse = Inf` to opt into the historical
unbounded behavior. The `puny_encode` and `puny_decode` caches are
unbounded by design and can be disabled for workloads with very many
unique hosts.
