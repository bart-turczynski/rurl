...
unique url × core-option combinations (the cache stores the
option-independent parse core, keyed by url, protocol/scheme handling,
`www_handling`, and `tld_source`; set `max_full_parse = Inf` for the
