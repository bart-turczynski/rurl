...
`rurl` memoizes parse/domain/TLD work to speed repeated operations over large URL
vectors. Inspect, clear, and configure the caches:
...
The `full_parse` cache is unbounded by default (`max_full_parse = Inf`); set a
bound to cap its peak memory. The `domain` and `tld` caches grow with the
number of unique hosts and can be disabled for workloads with very many of
them.

## Public Suffix List Data

`rurl` ships with a processed copy of the
[Public Suffix List (PSL)](https://publicsuffix.org/) and never downloads it at
runtime.
