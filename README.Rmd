...
`puny_decode`. The `full_parse` cache is bounded at 100000 entries by default: a
hard reset-watermark that caps peak memory (once the bound is hit the cache
empties completely and rebuilds, rather than evicting per entry). Set
`max_full_parse = Inf` to opt into the historical unbounded behavior. The
