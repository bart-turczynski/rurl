...
B_data_match <- data.frame(
  # One match with A_data_match.
  URL = c("http://example.com/page1/", "http://different.net/something"),
...
empty_df <- data.frame(
  URL = character(0),
  OtherCol = character(0),
  stringsAsFactors = FALSE
) # Ensure OtherCol for structure
...
test_that("permutation_join works with a basic match", {
  ...
  expect_named(
    res,
    c("A_data_match", "B_data_match", "JoinKey", "OtherColA_A", "OtherColB_B"),
    ignore.order = TRUE
  )
...
test_that("permutation_join handles no matches", {
  ...
  expect_s3_class(res, "data.frame")
  # Expect specific names even for 0 rows, based on
  # permutation_join_empty_result_structure
  expect_named(
    res,
    c(
      "A_data_no_match", "B_data_no_match", "JoinKey",
      "OtherColA_A", "OtherColB_B"
    ),
    ignore.order = TRUE
  )
...
test_that("permutation_join handles empty data_A input", {
  res <- permutation_join(
    ...
    col_A = "URL",
    col_B = "URL"
  ) # Specify cols for empty_df

  expect_s3_class(res, "data.frame")
  expect_named(
    res,
    c("empty_df", "B_data_match", "JoinKey", "OtherCol_A", "OtherColB_B"),
    ignore.order = TRUE
  )
  expect_equal(nrow(res), 0)
})
...
test_that("permutation_join handles empty data_B input", {
  ...
  expect_named(
    res,
    c("A_data_match", "empty_df", "JoinKey", "OtherColA_A", "OtherCol_B"),
    ignore.order = TRUE
  )
  expect_equal(nrow(res), 0)
})
...
test_that("permutation_join handles both inputs empty", {
  ...
  expect_named(
    res,
    c("empty_df", "JoinKey", "OtherCol_A", "OtherCol_B"),
    ignore.order = TRUE
  )
  expect_equal(nrow(res), 0)
})
...
test_that(
  "permutation_join warns for non-data.frame inputs",
  {
...
test_that("permutation_join warns for missing URL columns", {
  ...
  # A missing URL col
  expect_warning(
    # col_A=\"URL\" is default.
    res_A <- permutation_join(A_no_URL, B_data_match, col_A = "URL"),
  ...
  # B missing URL col
  expect_warning(
    # col_B=\"URL\" is default.
    res_B <- permutation_join(A_data_match, B_no_URL, col_B = "URL"),
...
test_that("permutation_join warns for URL columns of incorrect type", {
  ...
  expect_equal(
    length(names(res)),
    0
  ) # Expect character(0) for names due to early return
  expect_equal(nrow(res), 0)
})

test_that("permutation_join warns for data_B URL column of incorrect type", {
  B_URL_not_char <- data.frame(
    URL = 321,
    OtherColB = "valB",
    stringsAsFactors = FALSE
  )
...
test_that("permutation_join handles custom URL column names", {
  A_custom_col <- data.frame(
    MyURL_A = "http://example.com/custom_a",
    OtherColA = "valA",
    stringsAsFactors = FALSE
  )
  B_custom_col <- data.frame(
    MyURL_B = "http://example.com/custom_a",
    OtherColB = "valB",
    stringsAsFactors = FALSE
  )
  ...
  expect_named(
    res,
    c("A_custom_col", "B_custom_col", "JoinKey", "OtherColA_A", "OtherColB_B"),
    ignore.order = TRUE
  )
  expect_equal(nrow(res), 1)
...
test_that(
  "permutation_join handles overlapping column names with .x/.y merge suffixes",
  {
  A_dup <- data.frame(
...
test_that("permutation_join handles custom suffixes", {
  ...
  expect_named(
    res,
    c(
      "A_data_match", "B_data_match", "JoinKey",
      "OtherColA.left", "OtherColB.right"
    ),
    ignore.order = TRUE
  )
  expect_equal(nrow(res), 1)
...
# Test for when input data.frames have columns that would lead to name clashes
# after deparse(substitute())
# For example, if data_A is named 'df' and data_B is named 'df1'.
test_that(
  "permutation_join handles tricky deparse(substitute()) names",
  {
  ...
  # If both were list(), the internal empty structure might de-duplicate or
  # error.
  # The current empty structure function doesn't explicitly handle name_A ==
  # name_B.
  # Let's assume for now distinct deparsed names for simplicity in this test
  # edge case.
})
...
test_that("permutation_join works with piped inputs", {
  ...
  # This test is more about ensuring it runs; exact name checking for "." is
  # tricky.
  ...
  # Check that original values are there, regardless of exact 'A_pipe'/'B_pipe'
  # column name
  # This requires knowing which column corresponds to A_pipe's URL
  # This is complex to assert generally for piped inputs without knowing the
  # exact deparse result
