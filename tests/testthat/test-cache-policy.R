...
reset_caches <- function() {
  rurl_cache_config(
    full_parse = TRUE, domain = TRUE, tld = TRUE, max_full_parse = Inf
  )
  rurl_clear_caches()
}

test_that("rurl_cache_info reports the three caches with expected columns", {
  reset_caches()
  info <- rurl_cache_info()
  expect_s3_class(info, "data.frame")
  expect_equal(info$cache, c("full_parse", "domain", "tld"))
