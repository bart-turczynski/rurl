...
**Cell liveness.** Every cell in this register is **baseline-scoped**: a dated
statement measured at the commit it names, exactly like a decision-record body
after ADR 0014, and never a live figure. The envelope says append-only, and
that governs cells as well as rows. When a figure moves, the movement is
recorded by **appending a dated pointer** to the cell that names where the
current figure lives (OR-022's "Figure pointer" and OR-024's re-derivation
history are the pattern) — never by rewriting the number in place. A reader who
needs the current value follows the pointer to the consumer named in the row;
a reader who needs the value as it stood at a given commit reads the cell.
(RUL-012, RURL-dcrwibvm.)
...
| `imported` | `yes` iff the instance **vendors upstream bytes** — the single test `tests/testthat/fixtures/oracle-provenance.json`'s `section_2_3_scope` convention states for P5.3 §2.3, and the one RUL-011 binds this column to (P5.3 §2.3 then requires a provenance row); `no` if derived in-repo from a standard's text, from an accepted record, or from rurl itself — including expected values hand-derived here against upstream input *strings* transcribed by citation, which vendors nothing (OR-005). A non-imported row may still carry a voluntary provenance row; the gate requires one only for `yes`. |
...
| OR-005 | `tests/testthat/fixtures/url-standard-conformance.csv` — the WPT-sourced rows (of 75; `source_reference` records the exact upstream input string so a re-pin can re-locate them) | whatwg-wpt | no | WHATWG parse/serialize expectations for the represented inputs under `url_standard = "whatwg"` | Unrepresented cases; the RFC profile; and — per P5.3 §1.2 — the row's per-component `expected_host` / `expected_path` columns are the 18-field projection, valid for regression, never countable as conformance | `tests/testthat/test-url-standard-conformance.R :: url_standard conformance fixtures match pinned expectations` | | PROPOSED |
...
requirement rather than in violation of it. That is also why OR-005 is
`imported = no` (RUL-011, RURL-zxmzymom): hand-transcribing cited upstream
input *strings* without vendoring the upstream file is not an import under
§2.3's "did we vendor upstream BYTES" test, so the instance table and the
sidecar now say the same thing. Its provenance row above is **voluntary** —
kept because the verified-at citation is worth recording, not because §2.3
requires it — and the gate (O5) admits a voluntary row on a non-imported
instance while still refusing one whose `oracle_id` names no instance.

## Coverage of the taxonomy

| P5.3 §2 authority | instances | note |
|---|---|---|
| `whatwg-wpt` | 6 (OR-001, OR-005, OR-007, OR-021, OR-022, OR-024) | all six carry the revision pin the label's definition requires; OR-001, OR-005 and OR-007 took theirs from the sidecar record (RURL-hjgtwowf), and only OR-001's retrieval date and import command remain unrecorded (RURL-vwurxmzm). Five of the six are imports; OR-005 is `imported = no` (RUL-011) — its pin is a verified-at citation of upstream input strings, not a vendored artifact — so the authority count is six and the import count under P5.3 §2.3 is five. OR-024 re-extracts OR-022's upstream artifact at the same revision on the complementary `base` predicate, so it adds an axis (reference resolution) rather than a source |
