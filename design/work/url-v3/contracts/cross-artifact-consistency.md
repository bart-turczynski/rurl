...
| (v) | **No cell is unowned** across the artifact set: every open cell in every contract carries a named settlement destination. | The open-cell census below enumerates all 52 live open-cell IDs (+ G3.3's two frozen-text opens: one now closed by P1.2, one forwarded to OUT-O1); each row names an owning contract and a destination that is a sibling cell, the unmade P4 host record (RCON-08), the unmade P3 mutation/key-join owner decision, a future P5 decision, or §6 artifact 11 / G4. None resolves to "nobody." | PASS |
...
Every OPEN cell flagged by any sibling contract, with its owning contract and
named destination. No cell resolves to an unowned owner. Destinations that are
**outside G3** — the unmade P4 host record (RCON-08), the unmade P3
mutation/key-join owner decision, §6 artifact 11 (verification, RCON-09/10) / G4,
and future P5 performance decisions — are named as downstream, not as unowned.
This capstone introduces **no** new open cell; it only censuses these.
...
| KJ-O1 | key-join (G3.K) | the dedicated P3 key/join closure decision (default parsing standard) |
| KJ-O2 | key-join (G3.K) | the P3 key/join closure decision, coordinated with G3.H host identity (trailing-root-dot equivalence) |
| KJ-O3 | key-join (G3.K) | the P3 key/join closure decision (`url_right_join()` stable order) |
| KJ-O4 | key-join (G3.K) | the P3 key/join closure decision (non-keyable x rows in `url_anti_join()`) |
| KJ-O5 | key-join (G3.K) | the P3 key/join closure decision, then G3.4 records the exports (URL-column selector signature) |
| KJ-O6 | key-join (G3.K) | the P3 key/join closure decision (suffix and duplicate-name repair) |
| KJ-O7 | key-join (G3.K) | the P3 key/join closure decision (comparison-key visibility default) |
| KJ-O8 | key-join (G3.K) | the P3 key/join closure decision (type restoration + zero-row prototypes) |
...
### G3.3 (canonical-state) frozen-text open cells

G3.3 was authored (leaf merged) before P1.2 (authority split) and P2.2 were
accepted, so its frozen `## Open cells` section still lists two cells. The
capstone records their post-G3.3 disposition — this is precisely the cross-artifact
closure a consistency check surfaces:

| G3.3 open cell | disposition | destination |
|---|---|---|
| `authority_kind` — the `empty` value's operational meaning (S1-F5) | **NOW CLOSED** by the P1.2 authority split (`P1.2@bb3346e`): authority state splits into `authority_delimiter_present: logical` + `authority_payload_kind: {empty, present}`, which is exactly the resolution G3.3 Q3 deferred to "a dedicated P1 authority-state decision." G3.3's frozen text predates the acceptance. | closed — P1.2@bb3346e (the deferred P1 authority-state decision, now made) |
| Undivided `userinfo` source component vs the `user`/`password` split (S1 Q5) | still open; the `user`/`password` fields are SETTLED, only the augmenting undivided component is open. | G3.7 **OUT-O1** (the credential-handling clause of the output contract), driven by S1 Q5 |

### Census tally

52 live open-cell IDs across the nine contracts (CACHE 5, SCHEME 4, VAL 4,
CLEAN 1, MUT 12, OUT 5, HOST 8, KJ 8, PSC 5), plus G3.3's two frozen-text cells
(one closed by P1.2, one forwarded to OUT-O1). Every one carries a named
destination; **none is unowned.** PSC-O1..O5 are pure forwarders to sibling cells
(not independent product cells); HOST-O1..O8, MUT-O1..O12, and KJ-O1..O8 forward to
the two unmade owner decisions (the P4 host record / RCON-08 and the P3
mutation + P3 key/join closure decisions); CACHE-O1/O2/O3/O5 and VAL-O4 forward to
future P5 / post-3.0 decisions; CACHE-O4 and PSC-O5 forward to §6 artifact 11 / G4.
...
- **Downstream artifacts** — §6 artifact 11 (verification, RCON-09/10), the unmade
  P4 host record (RCON-08), and the unmade P3 mutation / P3 key-join closure
  decisions are named as census destinations; their contracts are authored
  elsewhere (G4 / a later P-tier slice), not here.
