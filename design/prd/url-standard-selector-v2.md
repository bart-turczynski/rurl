...
| D6 | `resolve_url()` | New public entry point, not a new divergence axis. Base-path merge (RFC 3986 §5.3: query-only ref, fragment-only ref, empty ref, scheme-relative ref, path-merge) is identical in both standards. `url_standard` flows through to the existing/planned path (T3/T5), host (T4), backslash (D2), and port (D1) logic rurl already has or is building. **Superseded in part (P2.7 D-B, `design/work/url-v3/decisions/P2.7-display-and-resolver-output.md`; RURL-fupsemxr, 2026-08-12):** the merge is **not** standard-agnostic. D6's conclusions stand - `resolve_url()` is a public entry point and `url_standard` does flow through the path/host/backslash/port axes - but the rationale cell is falsified by measurement, not by argument. Over the WHATWG's own base-carrying corpus (274 WPT base-relative success rows, `tests/testthat/test-wpt-base-relative.R`) the standard-blind merge scored 218 exact / 56 differing; four WHATWG reference-*parsing* rules that RFC 3986 §5 has no equivalent for - same-special-scheme references, backslash-as-slash and leading slash runs, C0-or-space stripping - now fire under `url_standard = "whatwg"` only, and the real RFC 3986 §3.1 scheme production replaces Appendix B's non-validating group under both named profiles. `url_standard = NULL` stays byte-frozen (ADR 0007, P2.7 D-C). Re-derived at T2.5: **247 exact / 27 differing**. | Decomposing the WHATWG base-URL algorithm against RFC 3986 §5.3 shows no genuinely new divergent behavior once the axes above exist - only a shared merge step plus composition of already-scoped axes. **Retired by P2.7 D-B:** it held only for the four axes D6 enumerated (path, host, backslash, port); the divergences are in a stage D6's decomposition never separated from the merge - parsing the *reference*. |
...
- Shares the base-merge algorithm (standard-agnostic) across both profiles. **Superseded in part - see the D6 note in §4:** the merge is standard-*aware*. Under `"whatwg"` the WHATWG reference-parsing rules run before the merge (P2.7 D-B); under `"rfc3986"` it is RFC 3986 §5.2-§5.3; under `NULL` it is byte-frozen (ADR 0007). Only the scheme production is shared by both named profiles, because RFC 3986 §3.1 and WHATWG agree on it.
