...
# Extract and process
tld_icann <- sort_by_depth(
  clean_lines(psl_raw[(icann_start + 1):(icann_end - 1)])
)
tld_private <- sort_by_depth(
  clean_lines(psl_raw[(private_start + 1):(private_end - 1)])
)
tld_all <- sort_by_depth(unique(c(tld_icann, tld_private)))

# Add punycode-encoded versions of each TLD
add_punycode <- function(x) {
  puny <- punycoder::puny_encode(x, strict = FALSE)
  puny[is.na(puny)] <- x[is.na(puny)]
  unique(c(x, puny))
}
...
# Extract punycode TLD (xn--...) and associated Unicode version
# (assumed to be the next non-comment line).
punycode_map <- lapply(seq_along(puny_lines), function(i) {
  ...
  unicode_line <- NA
  next_lines <- psl_raw[
    (which(psl_raw == puny_lines[i]) + 1):(which(psl_raw == puny_lines[i]) + 5)
  ]
  next_lines <- trimws(
    next_lines[!grepl("^//", next_lines) & nzchar(next_lines)]
  )
...
punycode_df <- punycode_df[
  !is.na(punycode_df$unicode) & nzchar(punycode_df$unicode),
]
