...
.derive_url_metadata_vec <- function(a, opts, n) {
  ...
  # Classify host_type as an (host, url_standard) function and emit the `ipv4-*`
  # diagnostics keyed to host SHAPE in BOTH modes ("facts not policy"). Reads
  # a$final_host / a$is_ip_host and opts$url_standard; sets host_type[...] and
  # diag <- .diag_add(diag, <mask>, "ipv4-...").
  ...
  # diag <- .diag_add(diag, <mask>, "encoded-dot-segment"/...).
